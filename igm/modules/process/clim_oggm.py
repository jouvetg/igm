...
def params_clim_oggm(parser):
    ...
    parser.add_argument(
        ...
        type=list,
        default=[["time", "delta_temp", "prec_scal"], [1900, 0.0, 1.0], [2020, 0.0, 1.0]],
...
def  initialize_clim_oggm(params,state):
    ...
    params.temp_default_gradient = oggm_mb_calib["mb_global_params"]["temp_default_gradient"]
    ...
    nc = Dataset(os.path.join(params.working_dir,params.oggm_RGI_ID,'climate_historical.nc'))
    ...
    if params.clim_oggm_clim_trend_array==[]:
        ...
        state.climpar = np.array(params.clim_oggm_clim_trend_array[1:]).astype(np.float32)
...
def update_clim_oggm(params,state):
    ...
    if ((state.t - state.tlast_clim_oggm) >= params.clim_oggm_update_freq):
        ...
        PREC = tf.expand_dims( tf.expand_dims(np.squeeze(state.prec[II,:]), axis=-1), axis=-1 )
        TEMP = tf.expand_dims( tf.expand_dims(np.squeeze(state.temp[II,:]), axis=-1), axis=-1 )
        TEMP_STD = tf.expand_dims( tf.expand_dims(np.squeeze(state.temp_std[II,:]), axis=-1), axis=-1 )
