...
def initialize(cfg, state):
    ...
    if cfg.output.plot2d.plt2d_editor == "vs":
    ...
    os.system("echo rm " + cfg.output.plot2d.plt2d_var + "*.png" + " >> clean.sh")
...
def run(cfg, state):
    if state.saveresult:
        ...
        if cfg.output.plot2d.plt2d_var == "velbar_mag":
...
#            alpha=0.65,
        ...
        if cfg.output.plot2d.plt2d_var=="velbar_mag":
            im = state.ax.imshow(
                np.where(state.thk > 0, vars(state)[cfg.output.plot2d.plt2d_var], np.nan),
                origin="lower",
                cmap="turbo",
                extent=state.extent, 
                norm=matplotlib.colors.LogNorm(vmin=1, vmax=cfg.output.plot2d.plt2d_var_max)
            )
        else:
            im = state.ax.imshow(
                np.where(state.thk > 0, vars(state)[cfg.output.plot2d.plt2d_var], np.nan),
                ...
                cmap='jet',
                vmin=0,
                vmax=cfg.output.plot2d.plt2d_var_max,
                extent=state.extent,
            )
        if cfg.output.plot2d.plt2d_particles:
        ...
        if not hasattr(state, "already_set_cbar"):
            state.cbar = plt.colorbar(im, label=cfg.output.plot2d.plt2d_var)
            state.already_set_cbar = True

        if cfg.output.plot2d.plt2d_live:
            if cfg.output.plot2d.plt2d_editor == "vs":
            ...
            plt.savefig(
                cfg.output.plot2d.plt2d_var + "-" + str(state.t.numpy()).zfill(4) + ".png",
                bbox_inches="tight",
                pad_inches=0.2,
            )
