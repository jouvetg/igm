...
def params(parser):
    ...
    parser.add_argument(
        ...
        type=float,
        default=0.0032967032967033 * 365.242198781,
        help="Degree-day factor for snow (ice eq.) (unit: meter / (Kelvin year))",
    )
    parser.add_argument(
        "--smb_accpdd_melt_factor_ice",
        type=float,
        default=0.00879120879120879 * 365.242198781,
        help="Degree-day factor for ice (ice eq.) (unit: meter / (Kelvin year))",
    )
    parser.add_argument(
...
def update(params, state):
    ...
    Input:  state.precipitation [Unit: m ice eq. / y]
    ...
    if (state.t - state.tlast_mb) >= params.smb_accpdd_update_freq:
        ...
        accumulation /= accumulation.shape[
            0
        ]  # unit to [ m ice eq. / y ] -> [ m ice eq. ]
        pos_temp_year /= pos_temp_year.shape[0]  # unit to [ °C ]            -> [ °C y ]
        ...
        for kk in range(state.air_temp.shape[0]):
            ...
            # the ablation (unit is m ice eq.) is the product of positive temp  with melt
