...
def run(cfg, state) -> xr.Dataset:
    with xr.open_dataset(cfg.input.local.filename) as f: # make agnostic to the input module (local etc.)
        ds = f.load()
    
    if "time" in ds.dims:
        state.logger.info(f"Time dimension found. Selecting the first time step at {cfg.modules.time_igm.time_start}")
        ds = ds.sel(time=ds.time[cfg.modules.time_igm.time_start])
    ...
    if crop:
        ...
        ds = ds.sel(x=slice(cfg.input.local.crop.xmin, cfg.input.local.crop.xmax),
                    y=slice(cfg.input.local.crop.ymin, cfg.input.local.crop.ymax))
    
    max_coarsening_ratio = max(cfg.input.local.coarsening.ratio.x, cfg.input.local.coarsening.ratio.y)
    if max_coarsening_ratio > 1:
        state.logger.info("Coarsening dataset")
        ds = ds.coarsen(x=cfg.input.local.coarsening.ratio.x,
                             y=cfg.input.local.coarsening.ratio.y,
                             boundary=cfg.input.local.coarsening.boundary).mean()
...
def complete_data(ds: xr.Dataset) -> xr.Dataset:
    ...
    ds["dx"] = ds.x.data[1] - ds.x.data[0]
    ds["dy"] = ds.y.data[1] - ds.y.data[0]
