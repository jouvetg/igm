...
def initialize_pretraining(params, state):
    ...
    subdatasetname_test, subdatasetpath_test = \
        _findsubdata(  os.path.join(params.data_dir, "test") ) 
...
def compute_solutions(params,state):
    ...
    for par in state.PAR:
        ...
        np.savetxt( os.path.join(path, "costs-stokes.dat"), np.stack(MISFIT), fmt="%.5f", )
...
def train_iceflow_emulator(params,state,trainingset,augmentation=True):
    ...
    for epoch in range(params.epochs):
        ...
        for p in nsub: 
            ...
            if (params.soft_begining>0) & (params.soft_begining<epoch):
                co      = int(2**tf.random.uniform(shape=[1], minval=params.min_coarsen, maxval=params.max_coarsen, dtype=tf.int32))
                val_A   = tf.random.uniform(shape=[1],minval=params.min_arrhenius, maxval=params.max_arrhenius)
                val_C   = tf.random.uniform(shape=[1],minval=params.min_slidingco, maxval=params.max_slidingco)
            else:
            ...
            thk       = _aug( tf.expand_dims( tf.convert_to_tensor(ds['thk'])[it::st,::co,::co], axis=-1) , ri)[:bs,:,:,0]
            usurf     = _aug( tf.expand_dims( tf.convert_to_tensor(ds['usurf'])[it::st,::co,::co], axis=-1), ri)[:bs,:,:,0]
            ...
            optimizer.apply_gradients(zip(grads, state.iceflow_model.trainable_variables))
        ...
        if epoch % params.freq_test == 0:
            ...
            for (par,sol,sol_co) in zip(state.PAR,state.solutions,state.solutions_cost):
                ...
                if epoch % (params.epochs//20) == 0:
                    _plot_iceflow_Glen(params, state, X,Y,YP,str(epoch).zfill(5),path)
                ...
                print(datetime.datetime.now().strftime("%H:%M:%S"), ' TEST :', epoch, code, nl1, nl2, nbarl1, nbarl1a, MIS_CO[-1], optimizer.lr.numpy())
                
                fid = open(os.path.join(path, "misfit-pinn.dat"), "a")
                fid.write("%.0f %.4f %.4f %.4f %.4f %.4f \n" % (epoch, nl1, nl2, nbarl1, nbarl1a, MIS_CO[-1]))
                fid.close()
    ...
    for l,par in enumerate(state.PAR):
        p    = par[0]
        code = p.split('/')[-1] + "_" + str(par[1]) + '_A' + str(int(par[2])) + '_C' + str(int(par[3]*100)/100) + '_R' + str(int(par[4]))
        plt.plot(params.freq_test*np.arange(state.MISFIT.shape[0]), state.MISFIT[:, l], label="MISFIT " + code)
    plt.ylim(0, 1)
    plt.legend()
    plt.tight_layout()
    ...
    for l,par in enumerate(state.PAR):
        p    = par[0]
        code = p.split('/')[-1] + "_" + str(par[1]) + '_A' + str(int(par[2])) + '_C' + str(int(par[3]*100)/100) + '_R' + str(int(par[4]))
        plt.plot(params.freq_test*np.arange(state.MISFIT_CO.shape[0]), state.MISFIT_CO[:, l], label="MISFIT " + code)
    plt.ylim(0, 1)
    plt.legend()
    plt.tight_layout()
...
#    vt   = Y[0,:,:,2*N-1]  ; #tf.reduce_mean( Y[0,:,:,N:] , axis=-1) 
    ...
    velbar_magt = tf.norm( tf.concat([tf.expand_dims(ut, axis=-1), 
                           tf.expand_dims(vt, axis=-1)], axis=2),  axis=2,  )
...
#    vp   = YP[0,:,:,2*N-1] ; #vp   = tf.reduce_mean( YP[0,:,:,N:] , axis=-1)
    ...
    velbar_magt = tf.norm( tf.concat([tf.expand_dims(ut, axis=-1), tf.expand_dims(vt, axis=-1)], axis=2),  axis=2,  )
    velbar_magp = tf.norm( tf.concat([tf.expand_dims(up, axis=-1), tf.expand_dims(vp, axis=-1)], axis=2),  axis=2,  )
