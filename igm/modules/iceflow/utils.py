    ...
    if not hasattr(state, "arrhenius"):
        if cfg.iceflow.iflo_dim_arrhenius == 3:
            state.arrhenius = tf.Variable(
                tf.ones((cfg.iceflow.iflo_Nz, state.thk.shape[0], state.thk.shape[1]))
                * cfg.iceflow.iflo_init_arrhenius * cfg.iceflow.iflo_enhancement_factor, trainable=False
            )
        else:
            state.arrhenius = tf.Variable(
                tf.ones_like(state.thk) * cfg.iceflow.iflo_init_arrhenius * cfg.iceflow.iflo_enhancement_factor, trainable=False
            )

    if not hasattr(state, "slidingco"):
        state.slidingco = tf.Variable(
            tf.ones_like(state.thk) * cfg.iceflow.iflo_init_slidingco, trainable=False
        )

    # here we create a new velocity field
    if not hasattr(state, "U"):
        state.U = tf.Variable(
            tf.zeros((cfg.iceflow.iflo_Nz, state.thk.shape[0], state.thk.shape[1])), trainable=False
        )
        state.V = tf.Variable(
            tf.zeros((cfg.iceflow.iflo_Nz, state.thk.shape[0], state.thk.shape[1])), trainable=False
        )
...
def define_vertical_weight(cfg, state):
    ...
    zeta = np.arange(cfg.iceflow.iflo_Nz + 1) / cfg.iceflow.iflo_Nz
    weight = (zeta / cfg.iceflow.iflo_vert_spacing) * (
        1.0 + (cfg.iceflow.iflo_vert_spacing - 1.0) * zeta
    )
...
def compute_PAD(cfg,Nx,Ny):
    ...
    if cfg.iceflow.iflo_multiple_window_size > 0:
        NNy = cfg.iceflow.iflo_multiple_window_size * math.ceil(
            Ny / cfg.iceflow.iflo_multiple_window_size
        )
        NNx = cfg.iceflow.iflo_multiple_window_size * math.ceil(
            Nx / cfg.iceflow.iflo_multiple_window_size
        )
