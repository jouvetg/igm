...
def finalize_anim_plotly(params, state):
    ...
    app.layout = html.Div([
        ...
        html.Div([
            ...
            html.Div(children=[dcc.Dropdown(['thickness [m]', 'velocity [m/a]', 'SMB [m]'],
                                            'thickness [m]', id='property')], style={'margin-bottom': '10px'}),
            # slider for camera-position in x-y-plane
            html.Div(dcc.Slider(id='camera_angle', step=9, value=45, min=0, max=180, marks=None, drag_value=45,
                                included=False)),
            # 3D surface plot
        ...
        html.Div(children=[dcc.Slider(id='camera_height', step=0.2, value=1.5, min=0, max=2, vertical=True,
                                      drag_value=1.5, marks=None, verticalHeight=600, included=False)],
                 style={'width': '5%', 'display': 'inline-block', 'text_align': 'center'})
    ],
    ...
    def updata_graph(property, camera_angle, camera_height):
        ...
        for i, year in enumerate(time):
            ...
            bottom_fig = go.Surface(z=glacier_bottom[::res, ::res], x=lat_range[::res], y=lon_range[::res],
                                    colorscale=color_scale, cmax=max_property_map, cmin=min_property_map,
                                    surfacecolor=property_map[::res, ::res], showlegend=True, name='glacier bottom',
                                    colorbar=dict(title=property, titleside='right'), )

            # create 3D bedrock plots
            bedrock_fig = go.Surface(z=bedrock[::res, ::res], x=lat_range[::res], y=lon_range[::res],
                                     colorscale=custom_colorscale,
                                     opacity=1, showlegend=True, name='bedrock', cmax=max_bedrock, cmin=0,
                                     colorbar=dict(x=-.1, title="elevation [m]", titleside='right'))
