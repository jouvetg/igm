...
def init_iceflow_v1(params,self):
    ...
    if not hasattr(self, "strflowctrl"):
        self.strflowctrl = tf.Variable(
            tf.ones_like(self.thk) * params.init_strflowctrl
        )

    if not hasattr(self, "arrhenius"):
        self.arrhenius = tf.Variable(
            tf.ones_like(self.thk) * params.init_arrhenius
        )

    if not hasattr(self, "slidingco"):
        self.slidingco = tf.Variable(
            tf.ones_like(self.thk) * params.init_slidingco
        )
    ...
    if params.multiple_window_size > 0:
        NNy = params.multiple_window_size * math.ceil(
            Ny / params.multiple_window_size
        )
        NNx = params.multiple_window_size * math.ceil(
            Nx / params.multiple_window_size
        )
...
def update_iceflow_v1(params,self):
    ...
    self.slopsurfx, self.slopsurfy = compute_gradient_tf(
