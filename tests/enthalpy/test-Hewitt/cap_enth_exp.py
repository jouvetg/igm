...
from igm.modules.process.enthalpy import vertically_discretize_tf
import igm


def params_cap_enth_exp(parser):
    igm.params_time(parser)
    igm.params_iceflow(parser)


def initialize_cap_enth_exp(params, state):
    ...
    igm.define_vertical_weight(params, state)

    igm.update_2d_iceflow_variables(params, state)


def update_cap_enth_exp(params, state):
    state.dt = 1.0
...
def finalize_cap_enth_exp(params, state):
    pass
