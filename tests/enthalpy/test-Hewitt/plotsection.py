...
def initialize_plotsection(params, state):
    ...
    os.system(
        "echo rm "
        + os.path.join(params.working_dir, "temperature*.png")
        + " >> clean.sh"
    )


def update_plotsection(params, state):
    if state.saveresult:
...
def finalize_plotsection(params, state):
    pass
