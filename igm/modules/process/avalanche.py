...
def update_avalanche(params, state):
    ...
    if (state.t - state.tlast_avalanche) >= params.avalanche_update_freq:
        ...
        Zb = state.topg
        Zi = Zb + H
        dHRepose = state.dx*tf.math.tan(params.avalanche_angleOfRepose*np.pi/180.)
        Ho = tf.maximum( H, 0 )
        ...
        while True:
            ...
            dZidx_down = tf.pad( tf.maximum(Zi[:,1:] - Zi[:,:-1],0), [[0,0],[1,0]],"CONSTANT")
            dZidx_up   = tf.pad( tf.maximum(Zi[:,:-1] - Zi[:,1:],0), [[0,0],[0,1]],"CONSTANT")
            dZidx      = tf.maximum(dZidx_down, dZidx_up) 

            dZidy_left    = tf.pad( tf.maximum(Zi[1:,:] - Zi[:-1,:],0), [[1,0],[0,0]],"CONSTANT")
            dZidy_right   = tf.pad( tf.maximum(Zi[:-1,:] - Zi[1:,:],0), [[0,1],[0,0]],"CONSTANT")
            ...
            delHup   = tf.pad( delH[:, :-1] * dZidx_up[:, :-1] / gradT[:, :-1], [[0,0],[1,0]],"CONSTANT")
            delHdn   = tf.pad( delH[:,1:] * dZidx_down[:,1:] / gradT[:,1:],     [[0,0],[0,1]],"CONSTANT")
            delHrt   = tf.pad( delH[:-1,:] * dZidy_right[:-1,:] / gradT[:-1,:], [[1,0],[0,0]],"CONSTANT")
            delHlt   = tf.pad( delH[1:,:] * dZidy_left[1:,:] / gradT[1:,:],     [[0,1],[0,0]],"CONSTANT")
