...
def finalize_print_comp(params, state):
    ...
    size_of_tensor = dict(sorted(size_of_tensor.items(), key=lambda item: item[1], reverse=True))

    print("Memory statistics report:")
    with open(
        os.path.join(params.working_dir, "memory-statistics.txt"), "w"
    ) as f:
...
def _plot_memory_pie(params, state):
    ...
    os.system(
        "echo rm "
        + os.path.join(params.working_dir, "memory-pie.png")
        + " >> clean.sh"
    )